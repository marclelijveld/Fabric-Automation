...
# --- Exclude auto date/time relationships from the scoring ------------------
# When "Auto date/time" is enabled in a Power BI semantic model, Power BI
# generates a hidden date dimension per date column, named
# "LocalDateTable_<guid>" (and one "DateTableTemplate_<guid>" template).
# The author never designed those tables or their relationships, and the
# feature itself is considered a bad practice. Including them inflates the
# relationship count and drags every relationship-based score down.
#
# Any relationship whose From- or To-table name contains "LocalDateTable_"
# (or "DateTableTemplate_") is therefore reported separately and skipped
# from all of this category's tests.
_AUTO_DATE_MARKERS = ("LocalDateTable_", "DateTableTemplate_")

def _is_auto_date_table(name: str) -> bool:
    if not isinstance(name, str):
        return False
    return any(m in name for m in _AUTO_DATE_MARKERS)

def _touches_auto_date(rel: dict) -> bool:
    return _is_auto_date_table(rel["fromTable"]) or _is_auto_date_table(rel["toTable"])
...
auto_date_tables = sorted({
    t for r in auto_date_relationships for t in (r["fromTable"], r["toTable"])
    if _is_auto_date_table(t)
})

if auto_date_relationships:
    print(
        f"Excluding {len(auto_date_relationships)} auto date/time "
        f"relationship(s) targeting {len(auto_date_tables)} auto-generated "
        f"date table(s) (names contain 'LocalDateTable_' / "
        f"'DateTableTemplate_'). These are ignored in all scoring for this "
        f"category as they are generated by the 'Auto date/time' feature "
        f"and were not authored by the modeller."
    )
    for r in auto_date_relationships:
        print(f"  - {r['fromTable']}[{r['fromColumn']}] -> "
              f"{r['toTable']}[{r['toColumn']}]")
else:
    print("No auto date/time relationships detected.")
